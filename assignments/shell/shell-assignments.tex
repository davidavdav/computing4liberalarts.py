...
The University College Utrecht (UCU) Accents project is a longitudinal study in accent convergence and accommodation in speech amongst students at UCU.  It involves the recording of speech from students at five different moments during their three-year undergraduate degree: at the very beginning in the first semester, and in the 2nd, 3rd, 4th and 6th semester.  UCU students originate from all over the world, but a significant fraction of about 60\,\% is Dutch.  The language spoken at the campus is English, both during and after classes.  
...
~/ucu$ 
\end{verbatim}
In these assignments I'll indicate the prompt---whatever your current working directory is---as |\$|, and you're not supposed to type that character as part of the command. 
...
The idea of the pipeline is that the |cat| (``concatenate'') command takes the content of |list.txt| and ``puts it into'' a pipe (represented by the symbol ``\texttt{\char`\|}'').  The command |wc|, in turn, reads from that pipeline again, and does its thing, i.e., count the words. 
...
\begin{explain}
  ...
  \item [|-r|] option: recursively.  If any of the items encountered is a directory, |cd| to this directory and recorsively remove all files and directories in there. 
...
\end{explain}
The reason that you can't just say ``|rm aup|'' is that it is full of stuff---if you would remove the directory |aup| but not the contents, then these contents would hang in the air and the filesystem's integrity would be violated: all files and directories must have a place on the filesystem.  
...
A slightly different variant to the above is the use of ``back quotes'', the ``|`|'' character that you often have to look for on the keyboard: it is usually snuggled away somewhere at the periphery, as if the designers of the keyboard forgot about it at first an had to fix that later.
\begin{verbatim}
...
\begin{exercise}
  ...
  \Question{Starting from |c|, craft a pipeline that generates the desired output.  Save this in |d|.}
  ...
  \Question{Recall the command line that produced |c|, and see if there are any files left---save in |c.dup|---these are the duplicates.}
\end{exercise}
