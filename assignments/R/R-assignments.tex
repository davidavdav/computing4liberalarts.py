...
\newenvironment{answer}{\def\FrameCommand{\colorbox{answercolor}}\MakeFramed{\FrameRestore}}{\endMakeFramed}
...
\begin{exercise}
  Use your proviously obtained skills with the command line to quickly find out the follinging quantities (|cat|, |wc| and |grep| are you friends).  If |bash| or the equivalent PowerShell commands are not available, skip this question, and offer a bounty of some kind to a person who can provide you with the answers.  
...
\begin{answer}
  ...
  \item A speaker is chosen randomly from the available speakers. The early participants were available for more listeners to be choseon randomly, this explains the density going down. 
  \end{enumerate}
\end{answer}
...
\begin{verbatim}
> sevens <- paste("q", 53:74, sep="") ## paste() glues the "q" to "53", etc.
> sevens
\end{verbatim}
